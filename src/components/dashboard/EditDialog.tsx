"use client";
import { setEditDialogClose, setPasswordEditFalse, setPasswordEditTrue, setUsernameEditFalse, setUsernameEditTrue } from "@/redux/slices/slice";
...
export default function EditDialog() {
  ...
  async function handleProfileUpdate() {
    ...
    try {
        ...
        if (usernameEdit) {
            const res = await mutation({usernameEdit : true,passwordEdit :false,data : {newUsername : username}});
            if (res.data.success) {
        ...
        if (passwordEdit) {
            const res = await mutation({usernameEdit : false,passwordEdit :true,data : {newPassword : password}});
            if (res.data.success) {
              toast(res.data.message,{type : "success"})
            }else {
              toast(res.data.message,{type : "error"})
        ...
        if (usernameEdit && passwordEdit) {
          const res = await mutation({usernameEdit : true,passwordEdit : true,data : {newUsername : username,newPassword : password}});
          if (res.data.success) {
            toast(res.data.message,{type : "success"})
          }else {
            toast(res.data.message,{type : "error"})
        ...
        <div className="flex-1">
          <div className="mt-5 flex flex-col ml-2 px-2">
            ...
            <div className="flex items-center">
              ...
              {usernameEdit ? (
                <Check onClick={handleUsernameUpdate} className=" ml-4 cursor-pointer" />
              ) : (
                <Edit
          ...
          <div className="mt-5 flex flex-col ml-2 px-2">
            ...
            <div className="flex items-center">
              ...
              {passwordEdit ? (
                <Check onClick={handlePasswordUpdate} className=" ml-4 cursor-pointer" />
              ) : (
                <Edit
                  className="mt-2 ml-2 cursor-pointer"
          ...
          <div className="w-full h-16 flex items-center justify-end mt-[80px] ">
              <button onClick={() => dispatch(setEditDialogClose())} className="border-btn">Cancel</button>
